    ...
    <article onClick={() => navigate(`/split/${split.id}`)}
      ...
      <div className="flex items-center gap-1.5 text-[12px] text-gray-400 font-medium mb-3 flex-wrap">
        ...
        {split.preferred_date && (
          <><span className="w-1 h-1 rounded-full bg-gray-300"/><Calendar size={12}/> {formatDate(split.preferred_date)}</>
        )}
      </div>
      ...
      <div className="flex items-center mb-3">
        ...
        {left > 0 && (
          <span className="text-[11px] font-bold ml-2 px-2 py-0.5 rounded-full bg-amber-50 text-amber-600 border border-amber-200">
            {left} spot{left !== 1 ? 's' : ''} left
          </span>
        )}
        {left === 0 && (
          <span className="text-[11px] font-bold ml-2 px-2 py-0.5 rounded-full bg-gray-100 text-gray-500">Full</span>
        )}
      </div>

      <div className="flex items-center justify-between">
        <div className="text-[12px] text-[#0f7a4b] font-semibold">
          Instead of £{split.total_price} alone
        </div>
        {left > 0 && (
