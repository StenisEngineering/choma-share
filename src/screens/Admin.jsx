    ...
    <div style={{
      ...
      <div className="px-5 pb-4 flex-shrink-0"
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <p className="text-[17px] font-bold uppercase tracking-widest" style={{ color: '#c8f26d' }}>
              Admin Panel · Creovate Global Ltd
            </p>
            <h1 className="font-display font-black text-[25px] text-white tracking-tight">
              Choma Share
            </h1>
          </div>
          ...
          {TABS.map(t => {
            ...
            return (
              <button key={t.key} onClick={() => setTab(t.key)}
                className="flex-1 flex flex-col items-center gap-1 py-2 rounded-xl text-[17px] font-bold transition-all"
                style={{
...
function StatsTab({ toast }) {
  ...
  return (
    <div className="space-y-4">
      <h2 className="font-display font-bold text-[21px] text-gray-900">Overview</h2>
      ...
      <div className="grid grid-cols-2 gap-3">
        {[
          ...
          <div key={s.l} className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm">
            ...
            <div className="text-[17px] text-gray-400 font-semibold mt-1 uppercase tracking-wide">{s.l}</div>
          </div>
        ))}
      </div>

      <div className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm">
        <h3 className="font-bold text-[17px] text-gray-900 mb-3">Split Status Breakdown</h3>
        {[
          ...
          <div key={s.l} className="flex items-center gap-3 mb-2">
            <div className="text-[17px] font-semibold text-gray-600 w-20">{s.l}</div>
            <div className="flex-1 bg-gray-100 rounded-full h-2">
              <div className="h-2 rounded-full transition-all"
                style={{ width: stats.splits ? `${(s.v / stats.splits) * 100}%` : '0%', background: s.c }}/>
            </div>
            <div className="text-[17px] font-bold text-gray-700 w-6 text-right">{s.v}</div>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h2 className="font-display font-bold text-[21px] text-gray-900">Stores & Items</h2>
...
function SplitsTab({ toast }) {
  ...
  return (
    <div className="space-y-3">
      <h2 className="font-display font-bold text-[21px] text-gray-900">All Splits</h2>

      {/* Filter */}
      <div className="flex gap-2 flex-wrap">
        {['all','open','full','done','cancelled'].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className="text-[17px] font-bold px-3 py-1.5 rounded-full border transition-all capitalize"
            style={filter === f
        ...
        <div key={split.id} className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm">
          <div className="flex items-start justify-between mb-2">
            <div className="flex-1 min-w-0">
              <div className="font-bold text-[17px] text-gray-900 truncate">{split.title}</div>
              <div className="text-[17px] text-gray-400 mt-0.5">
            ...
            <span className="text-[17px] font-bold px-2 py-1 rounded-full ml-2 flex-shrink-0 capitalize"
          ...
          <div className="flex gap-2 flex-wrap">
            {split.status !== 'done' && (
              <button onClick={() => updateStatus(split.id, 'done')}
                className="text-[17px] font-bold px-2.5 py-1.5 rounded-xl flex items-center gap-1"
            ...
            <button onClick={() => deleteSplit(split.id)}
              className="text-[17px] font-bold px-2.5 py-1.5 rounded-xl flex items-center gap-1 bg-red-50 text-red-500">
...
function UsersTab({ toast }) {
  ...
  return (
    <div className="space-y-3">
      <h2 className="font-display font-bold text-[21px] text-gray-900">
        Users ({users.length})
      </h2>

      <input value={search} onChange={e => setSearch(e.target.value)}
        placeholder="Search by name or city..."
        className="w-full bg-white border border-gray-200 rounded-2xl px-4 py-3 text-[17px] outline-none focus:border-[#0f7a4b]"/>
      ...
      {loading ? <Spinner/> : filtered.map(u => (
        <div key={u.id} className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="font-bold text-[17px] text-gray-900">{u.name}</div>
              <div className="flex items-center gap-2 text-[17px] text-gray-400 mt-0.5">
                <MapPin size={10}/> {u.city}
                <span>· {u.total_splits ?? 0} splits</span>
                <span>· Score: {u.reliability_score ?? 5}</span>
              </div>
              <div className="text-[17px] text-gray-300 mt-0.5 truncate">
          ...
          <div className="flex items-center gap-2 mt-3">
            <span className="text-[17px] text-gray-500 font-semibold">Reliability:</span>
            {[1,2,3,4,5].map(n => (
              <button key={n} onClick={() => updateScore(u.id, n)}
                className="w-7 h-7 rounded-full text-[17px] font-bold transition-all"
                style={{
        ...
        {editing ? (
          <div className="space-y-2">
            <input value={storeName} onChange={e => setStoreName(e.target.value)}
              className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 text-[17px] font-semibold outline-none focus:border-[#0f7a4b]"
            ...
            <div className="flex gap-2">
              <button onClick={saveStore} disabled={saving}
                className="flex-1 py-2 rounded-xl text-[17px] font-bold text-white flex items-center justify-center gap-1"
                style={{ background: G }}>
                {saving ? '...' : <><Check size={14}/> Save</>}
              </button>
              <button onClick={() => setEditing(false)}
                className="flex-1 py-2 rounded-xl text-[17px] font-bold text-gray-600 bg-gray-100">
                Cancel
              </button>
            </div>
          </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <h3 className="font-bold text-[17px] text-gray-900">{store.name}</h3>
              ...
              <p className="text-[17px] text-gray-400">{store.address}</p>
              <p className="text-[17px] text-gray-300 mt-0.5">{store.store_items?.length ?? 0} items</p>
            </div>
            <div className="flex gap-1">
      ...
      {expanded && (
        <div className="border-t border-gray-100 px-4 pb-4">
          <div className="flex items-center justify-between py-3">
            <span className="text-[17px] font-bold uppercase tracking-widest text-gray-400">Items</span>
            <AddItemButton storeId={store.id} onAdded={onRefresh} toast={toast}/>
          </div>
          <div className="flex flex-col gap-2">
            {store.store_items?.length === 0 && (
              <p className="text-[17px] text-gray-400 text-center py-3">No items yet</p>
            )}
  ...
  if (editing) return (
    <div className="bg-gray-50 rounded-2xl p-3 space-y-2">
      <input value={name} onChange={e => setName(e.target.value)}
        className="w-full bg-white border border-gray-200 rounded-xl px-3 py-2 text-[17px] font-semibold outline-none focus:border-[#0f7a4b]"
        placeholder="Item name"/>
      <div className="flex items-center gap-1">
        <span className="text-[17px] font-bold text-gray-500">£</span>
        <input type="number" value={price} onChange={e => setPrice(e.target.value)}
          className="flex-1 bg-white border border-gray-200 rounded-xl px-3 py-2 text-[17px] outline-none focus:border-[#0f7a4b]"
          placeholder="Price" step="0.50" min="0"/>
      </div>
      <div className="flex gap-2">
        <button onClick={save} disabled={saving}
          className="flex-1 py-2 rounded-xl text-[17px] font-bold text-white"
          style={{ background: G }}>
          {saving ? '...' : 'Save'}
        </button>
        <button onClick={() => setEditing(false)}
          className="flex-1 py-2 rounded-xl text-[17px] font-bold text-gray-600 bg-white border border-gray-200">
          Cancel
        </button>
      </div>
    </div>
    ...
    <div className="flex items-center gap-3 py-2 px-1 border-b border-gray-50 last:border-0">
      <div className="flex-1">
        <div className="flex items-center gap-2">
          ...
          {!item.available && (
            <span className="text-[17px] font-bold px-1.5 py-0.5 rounded-full bg-red-50 text-red-500">Hidden</span>
          )}
        </div>
        <span className="font-display font-bold text-[17px]" style={{ color: item.bulk_price > 0 ? G : '#f59e0b' }}>
...
function AddItemButton({ storeId, onAdded, toast }) {
  ...
  if (!open) return (
    <button onClick={() => setOpen(true)}
      className="flex items-center gap-1 px-3 py-1.5 rounded-xl text-[17px] font-bold text-white"
      style={{ background: G }}>
  ...
  return (
    <div className="w-full mt-2 bg-gray-50 rounded-2xl p-3 space-y-2">
      <input value={name} onChange={e => setName(e.target.value)}
        className="w-full bg-white border border-gray-200 rounded-xl px-3 py-2 text-[17px] outline-none focus:border-[#0f7a4b]"
...
function AddStoreButton({ onAdded, toast }) {
  ...
  if (!open) return (
    <button onClick={() => setOpen(true)}
      className="flex items-center gap-1 px-3 py-1.5 rounded-xl text-[17px] font-bold border"
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center" onClick={() => setOpen(false)}>
      <div className="w-full max-w-md bg-white rounded-t-3xl p-5" onClick={e => e.stopPropagation()}>
        ...
        <h3 className="font-display font-bold text-[21px] text-gray-900 mb-4">Add New Store</h3>
        <div className="space-y-3">
          <input value={name} onChange={e => setName(e.target.value)}
            className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-[17px] outline-none focus:border-[#0f7a4b]"
