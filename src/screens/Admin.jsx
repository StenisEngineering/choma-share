    ...
    <div style={{
      ...
      <div className="px-5 pb-4 flex-shrink-0"
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <p className="text-[16px] font-bold uppercase tracking-widest" style={{ color: '#c8f26d' }}>
          ...
          {TABS.map(t => {
            ...
            return (
              <button key={t.key} onClick={() => setTab(t.key)}
                className="flex-1 flex flex-col items-center gap-1 py-2 rounded-xl text-[16px] font-bold transition-all"
                style={{
...
function StatsTab({ toast }) {
  ...
  return (
    <div className="space-y-4">
      <h2 className="font-display font-bold text-[20px] text-gray-900">Overview</h2>
      ...
      <div className="grid grid-cols-2 gap-3">
        {[
          ...
          <div key={s.l} className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm">
            ...
            <div className="text-[16px] text-gray-400 font-semibold mt-1 uppercase tracking-wide">{s.l}</div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm">
        ...
        {[
          ...
          <div key={s.l} className="flex items-center gap-3 mb-2">
            <div className="text-[16px] font-semibold text-gray-600 w-20">{s.l}</div>
            <div className="flex-1 bg-gray-100 rounded-full h-2">
              <div className="h-2 rounded-full transition-all"
                style={{ width: stats.splits ? `${(s.v / stats.splits) * 100}%` : '0%', background: s.c }}/>
            </div>
            <div className="text-[16px] font-bold text-gray-700 w-6 text-right">{s.v}</div>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h2 className="font-display font-bold text-[20px] text-gray-900">Stores & Items</h2>
...
function SplitsTab({ toast }) {
  ...
  async function archiveSplit(id) {
    const { error } = await supabase.from('splits').update({ status: 'archived' }).eq('id', id)
    if (error) toast(error.message, 'error')
    else { toast('Split archived — hidden from users', 'success'); loadSplits() }
  }

  async function reopenSplit(id) {
    const { error } = await supabase.from('splits').update({ status: 'open' }).eq('id', id)
    if (error) toast(error.message, 'error')
    else { toast('Split reopened', 'success'); loadSplits() }
  }

  async function archiveAllExpired() {
    if (!window.confirm(`Archive ${expiredCount} expired splits? They will be hidden from users.`)) return
    const expiredIds = splits.filter(isExpired).map(s => s.id)
    const { error } = await supabase.from('splits')
      .update({ status: 'archived' }).in('id', expiredIds)
    if (error) toast(error.message, 'error')
    else { toast(`${expiredIds.length} expired splits archived`, 'success'); loadSplits() }
  }
  ...
  const STATUS_COLORS = {
    ...
    archived:  { bg: '#f5f3ff', color: '#7c3aed' },
  }

  // Detect expired — date passed but still open/full
  const today = new Date(); today.setHours(0,0,0,0)
  const isExpired = (s) => {
    if (!s.preferred_date) return false
    const d = new Date(s.preferred_date)
    return d < today && ['open','full'].includes(s.status)
  }
  const expiredCount = splits.filter(isExpired).length

  return (
    <div className="space-y-3">
      <h2 className="font-display font-bold text-[20px] text-gray-900">All Splits</h2>

      {/* Filter */}
      <div className="flex gap-2 flex-wrap">
        {['all','open','full','done','cancelled','archived'].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className="text-[16px] font-bold px-3 py-1.5 rounded-full border transition-all capitalize"
            style={filter === f
      ...
      {/* Expired banner */}
      {expiredCount > 0 && filter !== 'archived' && (
        <div className="flex items-center justify-between p-3 rounded-2xl"
          style={{ background: '#fef3c7', border: '1px solid #fcd34d' }}>
          <div>
            <div className="text-[12px] font-bold text-amber-800">
              ⚠️ {expiredCount} expired split{expiredCount !== 1 ? 's' : ''} still showing to users
            </div>
            <div className="text-[10px] text-amber-600 mt-0.5">
              Date has passed but status is still open or full
            </div>
          </div>
          <button onClick={archiveAllExpired}
            className="text-[10px] font-bold px-3 py-1.5 rounded-xl ml-2 flex-shrink-0"
      ...
      {loading ? <Spinner/> : filtered.map(split => (
        <div key={split.id}
          ...
          style={isExpired(split) ? { borderColor: '#fcd34d', borderWidth: 1.5 } : {}}>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {isExpired(split) && (
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded-full"
                    style={{ background: '#fef3c7', color: '#a16207' }}>Expired</span>
                )}
              </div>
              <div className="text-[11px] text-gray-400 mt-0.5">
                {split.store?.name || split.custom_store || 'Unknown store'} · By {split.creator?.name || 'Unknown'}
              </div>
              <div className="text-[11px] text-gray-400">
                {split.split_members?.length ?? 0}/{split.people_needed} joined
                {split.preferred_date && ` · ${new Date(split.preferred_date).toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'})}`}
              </div>
            </div>
            <span className="text-[10px] font-bold px-2.5 py-1 rounded-full ml-2 flex-shrink-0 capitalize"
          ...
          <div className="flex gap-2 flex-wrap mt-3 pt-3 border-t border-gray-50">
            ...
            {/* Mark done */}
            {!['done','archived'].includes(split.status) && (
              <button onClick={() => updateStatus(split.id, 'done')}
                className="text-[10px] font-bold px-2.5 py-1.5 rounded-xl flex items-center gap-1"
            ...
            {/* Mark full */}
            {split.status === 'open' && (
              <button onClick={() => updateStatus(split.id, 'full')}
                className="text-[10px] font-bold px-2.5 py-1.5 rounded-xl flex items-center gap-1"
                style={{ background: '#fffbeb', color: '#a16207' }}>
                <Users size={11}/> Mark Full
              </button>
            )}

            {/* Reopen */}
            {['cancelled','archived','done'].includes(split.status) && (
              <button onClick={() => reopenSplit(split.id)}
                className="text-[10px] font-bold px-2.5 py-1.5 rounded-xl flex items-center gap-1"
                style={{ background: '#eff6ff', color: '#1e40af' }}>
                <AlertTriangle size={11}/> Reopen
              </button>
            )}

            {/* Cancel */}
            {['open','full'].includes(split.status) && (
            ...
            {split.status !== 'archived' && (
              <button onClick={() => archiveSplit(split.id)}
                className="text-[10px] font-bold px-2.5 py-1.5 rounded-xl flex items-center gap-1"
                style={{ background: '#f5f3ff', color: '#7c3aed' }}>
                <ShoppingBag size={11}/> Archive
              </button>
            )}

            {/* Delete */}
            <button onClick={() => deleteSplit(split.id)}
              className="text-[10px] font-bold px-2.5 py-1.5 rounded-xl flex items-center gap-1 bg-red-50 text-red-500">
...
function UsersTab({ toast }) {
  ...
  return (
    <div className="space-y-3">
      <h2 className="font-display font-bold text-[20px] text-gray-900">
        Users ({users.length})
      </h2>
        ...
        <div key={u.id} className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 text-[16px] text-gray-400 mt-0.5">
                <MapPin size={10}/> {u.city}
                <span>· {u.total_splits ?? 0} splits</span>
                <span>· Score: {u.reliability_score ?? 5}</span>
              </div>
              <div className="text-[16px] text-gray-300 mt-0.5 truncate">
          ...
          <div className="flex items-center gap-2 mt-3">
            <span className="text-[16px] text-gray-500 font-semibold">Reliability:</span>
            {[1,2,3,4,5].map(n => (
              <button key={n} onClick={() => updateScore(u.id, n)}
                className="w-7 h-7 rounded-full text-[16px] font-bold transition-all"
                style={{
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <button onClick={saveStore} disabled={saving}
                className="flex-1 py-2 rounded-xl text-[16px] font-bold text-white flex items-center justify-center gap-1"
                style={{ background: G }}>
                {saving ? '...' : <><Check size={14}/> Save</>}
              </button>
              <button onClick={() => setEditing(false)}
                className="flex-1 py-2 rounded-xl text-[16px] font-bold text-gray-600 bg-gray-100">
                Cancel
              </button>
            </div>
          </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[16px] font-bold px-2 py-0.5 rounded-full"
                  style={store.active
              ...
              <p className="text-[16px] text-gray-300 mt-0.5">{store.store_items?.length ?? 0} items</p>
            </div>
            <div className="flex gap-1">
      ...
      {expanded && (
        <div className="border-t border-gray-100 px-4 pb-4">
          <div className="flex items-center justify-between py-3">
            <span className="text-[16px] font-bold uppercase tracking-widest text-gray-400">Items</span>
            <AddItemButton storeId={store.id} onAdded={onRefresh} toast={toast}/>
          </div>
          <div className="flex flex-col gap-2">
            {store.store_items?.length === 0 && (
              <p className="text-[16px] text-gray-400 text-center py-3">No items yet</p>
            )}
  ...
  if (editing) return (
    <div className="bg-gray-50 rounded-2xl p-3 space-y-2">
      <input value={name} onChange={e => setName(e.target.value)}
        className="w-full bg-white border border-gray-200 rounded-xl px-3 py-2 text-[16px] font-semibold outline-none focus:border-[#0f7a4b]"
        placeholder="Item name"/>
      <div className="flex items-center gap-1">
        <span className="text-[16px] font-bold text-gray-500">£</span>
        <input type="number" value={price} onChange={e => setPrice(e.target.value)}
          className="flex-1 bg-white border border-gray-200 rounded-xl px-3 py-2 text-[16px] outline-none focus:border-[#0f7a4b]"
          placeholder="Price" step="0.50" min="0"/>
      </div>
      <div className="flex gap-2">
        <button onClick={save} disabled={saving}
          className="flex-1 py-2 rounded-xl text-[16px] font-bold text-white"
          style={{ background: G }}>
          {saving ? '...' : 'Save'}
        </button>
        <button onClick={() => setEditing(false)}
          className="flex-1 py-2 rounded-xl text-[16px] font-bold text-gray-600 bg-white border border-gray-200">
          Cancel
        </button>
      </div>
    </div>
  ...
  return (
    <div className="flex items-center gap-3 py-2 px-1 border-b border-gray-50 last:border-0">
      <div className="flex-1">
        <div className="flex items-center gap-2">
          <span className="text-[16px] font-semibold text-gray-900">{item.name}</span>
          {!item.available && (
            <span className="text-[16px] font-bold px-1.5 py-0.5 rounded-full bg-red-50 text-red-500">Hidden</span>
          )}
        </div>
        <span className="font-display font-bold text-[16px]" style={{ color: item.bulk_price > 0 ? G : '#f59e0b' }}>
...
function AddItemButton({ storeId, onAdded, toast }) {
  ...
  if (!open) return (
    <button onClick={() => setOpen(true)}
      className="flex items-center gap-1 px-3 py-1.5 rounded-xl text-[16px] font-bold text-white"
      style={{ background: G }}>
  ...
  return (
    <div className="w-full mt-2 bg-gray-50 rounded-2xl p-3 space-y-2">
      <input value={name} onChange={e => setName(e.target.value)}
        className="w-full bg-white border border-gray-200 rounded-xl px-3 py-2 text-[16px] outline-none focus:border-[#0f7a4b]"
...
function AddStoreButton({ onAdded, toast }) {
  ...
  if (!open) return (
    <button onClick={() => setOpen(true)}
      className="flex items-center gap-1 px-3 py-1.5 rounded-xl text-[16px] font-bold border"
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center" onClick={() => setOpen(false)}>
      <div className="w-full max-w-md bg-white rounded-t-3xl p-5" onClick={e => e.stopPropagation()}>
        ...
        <h3 className="font-display font-bold text-[20px] text-gray-900 mb-4">Add New Store</h3>
        <div className="space-y-3">
          <input value={name} onChange={e => setName(e.target.value)}
