      ...
      <div className="bg-white px-5 pt-4 pb-4 border-b border-gray-100 flex-shrink-0">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-[16px] text-gray-400 mt-0.5">Your trusted buying groups</p>
          </div>
        ...
        <div className="flex bg-gray-100 rounded-2xl p-1 mt-3">
          {[['mine','My Circles'],['discover','Discover']].map(([key, label]) => (
            <button key={key} onClick={() => setTab(key)}
              className="flex-1 py-2 rounded-xl text-[16px] font-bold transition-all"
              style={{
              ...
              <div className="px-4 pt-4 pb-6">
                {myCircles.length === 0 ? (
                  <div className="bg-white border border-gray-100 rounded-3xl p-8 text-center shadow-sm">
                    ...
                    <p className="font-display font-bold text-[20px] text-gray-900 mb-2">No circles yet</p>
                    ...
                    <button onClick={() => setShowCreate(true)}
                      className="flex items-center gap-2 mx-auto text-white px-5 py-2.5 rounded-2xl font-bold text-[16px]"
                      style={{ background: G }}>
              ...
              <div className="px-4 pt-4 pb-6">
                {publicCircles.length === 0 ? (
                  <div className="bg-white border border-gray-100 rounded-3xl p-8 text-center shadow-sm">
                    ...
                    <p className="font-display font-bold text-[20px] text-gray-900 mb-2">No public circles yet</p>
                    <p className="text-[16px] text-gray-400">Be the first to create a public circle in your area.</p>
                  </div>
                ) : (
                  <div className="flex flex-col gap-3">
                      ...
                      <div key={circle.id}
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <h3 className="font-bold text-[16px] text-gray-900">{circle.name}</h3>
                            {circle.description && (
                              <p className="text-[16px] text-gray-400 mt-0.5 line-clamp-2">{circle.description}</p>
                            )}
                            <div className="flex items-center gap-2 mt-1">
                              <Globe size={11} color="#9ca3af"/>
                              <span className="text-[16px] text-gray-400">Public · {circle.city}</span>
                            </div>
                          </div>
                          <button onClick={() => joinCircle(circle.id)}
                            className="text-white text-[16px] font-bold px-3 py-1.5 rounded-full flex-shrink-0"
                            style={{ background: G }}>
                            Join
                          </button>
                        </div>
      ...
      <div className="p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            {circle?.description && (
              <p className="text-[16px] text-gray-400 mt-0.5">{circle.description}</p>
            )}
            <div className="flex items-center gap-3 mt-1">
              <div className="flex items-center gap-1">
                <Users size={11} color="#9ca3af"/>
                <span className="text-[16px] text-gray-400">{memberCount} member{memberCount !== 1 ? 's' : ''}</span>
              </div>
              <div className="flex items-center gap-1">
                {circle?.is_private
                ...
                <span className="text-[16px] text-gray-400">{circle?.is_private ? 'Private' : 'Public'}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-2 mt-3">
          <button onClick={copyInviteCode}
            className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-[16px] font-bold"
            style={{ background: '#f0fdf4', color: G, border: '1px solid #b6f0d4' }}>
            <Copy size={13}/> Copy Invite Link
          </button>
          <button onClick={onLeave}
            className="px-3 py-2.5 rounded-xl text-[16px] font-bold bg-red-50 text-red-500">
            Leave
          </button>
        </div>
      </div>
    ...
    <>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white rounded-t-3xl z-50 p-5 max-w-md mx-auto"
        ...
        <div className="space-y-3 mb-5">
          <div>
            <label className="text-[16px] font-bold uppercase tracking-widest text-gray-400 mb-1.5 block">Circle name</label>
          ...
          <div>
            <label className="text-[16px] font-bold uppercase tracking-widest text-gray-400 mb-1.5 block">Description (optional)</label>
          ...
          <div className="flex items-center justify-between bg-gray-50 border border-gray-200 rounded-2xl px-4 py-3">
            <div>
              <div className="text-[16px] font-semibold text-gray-900">Private circle</div>
              <div className="text-[16px] text-gray-400 mt-0.5">Only people with invite link can join</div>
            </div>
