    ...
    <div className="flex flex-col h-full bg-gray-50">
      ...
      <div className="bg-white px-5 pt-2 pb-3 border-b border-gray-100 flex-shrink-0">
        <div className="flex items-start justify-between">
          <div>
            <p className="text-[16px] text-gray-400 font-medium">{greet}</p>
          ...
          <div className="grid grid-cols-3 gap-2 mt-3">
            {[
              ...
              <div key={s.l} className="bg-gray-50 border border-gray-100 rounded-xl px-3 py-2.5">
                <div className="font-display font-bold text-[20px] leading-none" style={{ color: '#0f7a4b' }}>{s.v}</div>
                <div className="text-[16px] text-gray-400 font-semibold mt-0.5">{s.l}</div>
              </div>
            ))}
          </div>
        )}
      ...
      <div className="flex-1 overflow-y-auto scrollbar-none">
        ...
        {profile && profile.city !== 'Sunderland' && (
          <div className="mx-4 mt-4 p-4 rounded-3xl text-center relative overflow-hidden"
            ...
            <p className="text-[16px] mb-3" style={{ color: 'rgba(255,255,255,0.65)' }}>
              We're launching in Sunderland first. You'll be notified the moment Choma Share goes live in {profile.city}.
            </p>
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-[16px] font-bold"
        ...
        <div className="px-4 pt-4">
          <button onClick={() => navigate('/create')}
            ...
            <div className="text-left">
              ...
              <div className="text-[16px] text-gray-400 mt-0.5">Post an item · find your people nearby</div>
            </div>
        ...
        <div className="flex items-center justify-between px-4 pt-3 pb-1">
          ...
          <span className="text-[16px] font-semibold cursor-pointer" style={{ color: '#0f7a4b' }}
            onClick={() => navigate('/splits')}>See all</span>
        </div>

        {loading && <Spinner/>}
        {error && (
          <div className="mx-4 p-4 bg-red-50 border border-red-100 rounded-2xl text-[16px] text-red-600">{error}</div>
        )}
        ...
        {!loading && splits.length === 0 && (
          <div className="mx-4 mt-2 p-4 bg-white border border-gray-100 rounded-3xl text-center">
            ...
            <p className="text-[16px] text-gray-400">Be the first — create a split and share it with your community.</p>
          </div>
        )}
        ...
        <div className="mx-4 mb-3 p-3 rounded-2xl flex items-start gap-2.5"
          ...
          <p className="text-[16px] leading-relaxed" style={{ color: '#0f7a4b' }}>
        ...
        <div className="mx-4 mb-3 bg-white border border-gray-100 rounded-3xl overflow-hidden shadow-sm">
          {circles.length === 0 ? (
            <div className="px-4 py-5 text-center">
              <p className="text-[16px] text-gray-400 mb-3">No circles yet</p>
              <button
                onClick={() => navigate('/circles')}
                className="text-[16px] font-bold px-4 py-2 rounded-xl text-white"
            ...
            circles.map((m, i) => (
              <div
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-[16px] font-bold text-gray-900 truncate">{m.circle?.name}</div>
