  ...
  return (
    <div className="flex flex-col h-full bg-gray-50">
      ...
      <div className="bg-white px-5 pt-4 pb-4 border-b border-gray-100 flex-shrink-0">
        ...
        <p className="text-[16px] text-gray-400 mt-0.5">Splits you created or joined</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-none">
        ...
        {!loading && splits.length === 0 && (
          <div className="mx-4 mt-6 p-8 bg-white border border-gray-100 rounded-3xl text-center shadow-sm">
            ...
            <p className="font-display font-bold text-[20px] text-gray-900 mb-2">No splits yet</p>
            <p className="text-[16px] text-gray-400 mb-5">Create your first split and share it with your community.</p>
            <button onClick={() => navigate('/create')}
              className="flex items-center gap-2 mx-auto text-white px-6 py-3 rounded-2xl font-bold text-[16px]"
  ...
  return (
    <div onClick={() => navigate(`/split/${m.split_id}`)}
      ...
      <div className="flex justify-between items-start mb-2 mt-0.5">
        ...
        <span className="text-[16px] font-bold px-2.5 py-1 rounded-full ml-2 whitespace-nowrap"
          style={{ background: status.bg, color: status.color }}>
          {status.label}
        </span>
      </div>

      <div className="text-[16px] text-gray-400 mb-3">
        {split?.store?.name} · {split?.preferred_date ? formatDate(split.preferred_date) : 'Date TBC'}
      </div>

      <div className="flex items-center justify-between">
        <div className="text-[16px] font-semibold" style={{ color: '#0f7a4b' }}>
