...
import { MapPin, Bell, Phone, LogOut, Star, ChevronRight } from 'lucide-react'
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50">
      ...
      <div className="bg-white px-5 pt-6 pb-5 border-b border-gray-100 text-center flex-shrink-0">
        <div className="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-3 font-display font-bold text-3xl text-white"
        ...
        <div className="inline-flex items-center gap-2 rounded-full px-4 py-1.5" style={{background:'#ecfff5',border:'1px solid #b6f0d4'}}>
          <Star size={14} fill={G} color={G}/>
          <span className="font-display font-bold text-[17px] leading-none" style={{color:G}}>{profile.reliability_score ?? '5.0'}</span>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-none">
        ...
        <div className="grid grid-cols-3 gap-2.5 mx-4 mt-4">
          {[{v:profile.total_splits??0,l:'Splits'},{v:`£${profile.total_saved??0}`,l:'Saved'},{v:2,l:'Circles'}].map(s=>(
        ...
        <div className="mx-4 mt-4">
          <div className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-2">Trusted Shopper Badges</div>
          <div className="flex flex-wrap gap-2">
            {[
              {l:'Always Shows Up',bg:'#ecfff5',c:G,bc:'#b6f0d4'},
              {l:'Verified Phone',bg:'#fffbeb',c:'#a16207',bc:'#fde68a'},
              {l:'5+ Splits Done',bg:'#eff6ff',c:'#1e40af',bc:'#bfdbfe'},
            ].map(b=>(
              <span key={b.l} className="inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[11px] font-bold border"
        ...
        <div className="mx-4 mt-4">
          ...
          {loading ? <Spinner size={20}/> : splits.length===0 ? (
            <div className="bg-white border border-gray-100 rounded-2xl p-4 text-center text-[13px] text-gray-400 shadow-sm">No splits yet — create your first one!</div>
          ) : (
            <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
              {splits.slice(0,5).map((m,i,arr)=>(
                <div key={m.split_id} onClick={()=>navigate(`/split/${m.split_id}`)}
                  className={`flex items-center gap-3 px-4 py-3.5 cursor-pointer active:bg-gray-50 ${i<arr.length-1?'border-b border-gray-100':''}`}>
                  <div className="w-9 h-9 rounded-xl flex items-center justify-center text-lg flex-shrink-0" style={{background:'#ecfff5'}}>🛒</div>
                  <div className="flex-1 min-w-0">
                    <div className="text-[13px] font-semibold text-gray-900 truncate">{m.split?.title||'Split'}</div>
                    <div className="text-[11px] text-gray-400">{m.split?.store?.name}</div>
                  </div>
                  <span className="text-[10px] font-bold px-2 py-1 rounded-full"
                    style={m.split?.status==='open'?{background:'#ecfff5',color:G}:{background:'#f3f4f6',color:'#6b7280'}}>
                    {m.split?.status}
                  </span>
                </div>
              ))}
        ...
        <div className="mx-4 mt-4 mb-6">
          ...
          <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
            {[
              ...
              {icon:<Phone  size={17} color='#1e40af'/>,  bg:'#eff6ff', l:'Phone number',  v:'Verified ✓'},
            ].map((r,i)=>(
              <div key={r.l} className={`flex items-center gap-3 px-4 py-3.5 ${i<2?'border-b border-gray-100':''} active:bg-gray-50`}>
                ...
                <div className="text-[12px] text-gray-400">{r.v}</div>
                <ChevronRight size={15} color="#d1d5db"/>
              </div>
            ))}
            <button onClick={logout} className="w-full flex items-center gap-3 px-4 py-3.5 border-t border-gray-100 active:bg-red-50">
