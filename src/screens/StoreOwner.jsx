    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      <div className="w-full max-w-sm bg-white rounded-3xl shadow-xl overflow-hidden">
        <div className="px-6 pt-8 pb-6 text-center"
          ...
          <h1 className="font-display font-black text-[23px] text-white tracking-tight">Store Portal</h1>
        ...
        <form onSubmit={onLogin} className="px-6 py-5 space-y-3">
          <input type="email" value={email} onChange={e => setEmail(e.target.value)}
            ...
            className="w-full bg-gray-50 border border-gray-200 rounded-2xl px-4 py-3 text-[17px] outline-none focus:border-[#0f7a4b]"/>
          <input type="password" value={password} onChange={e => setPassword(e.target.value)}
            placeholder="Password"
            className="w-full bg-gray-50 border border-gray-200 rounded-2xl px-4 py-3 text-[17px] outline-none focus:border-[#0f7a4b]"/>
          <button type="submit" disabled={!email||!password||loading}
            className="w-full py-3.5 rounded-2xl text-[17px] font-bold text-white disabled:opacity-40"
            style={{ background: '#0f7a4b' }}>
            {loading ? <Spinner size={18} color="white"/> : 'Sign In'}
          </button>
        </form>
        <p className="text-center text-[17px] text-gray-400 pb-2 px-6">Not set up yet? Email <span style={{ color: '#0f7a4b' }}>hello@choma.app</span></p>
        <p className="text-center text-[17px] text-gray-300 pb-5">© 2026 Creovate Global Ltd</p>
      </div>
    </div>
  )
}
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="px-5 pt-10 pb-4 flex-shrink-0"
        ...
        <div className="flex items-center justify-between">
          <div>
            <p className="text-[17px] font-bold uppercase tracking-widest" style={{ color: '#c8f26d' }}>Store Portal</p>
            <h1 className="font-display font-black text-[23px] text-white tracking-tight">{store.name}</h1>
            <p className="text-[17px] mt-0.5" style={{ color: 'rgba(255,255,255,0.55)' }}>{store.address}</p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-2 mt-4">
          {[
            ...
            <div key={s.l} className="rounded-2xl p-3 text-center"
              ...
              <div className="font-display font-bold text-[25px] text-white">{s.v}</div>
      ...
      <div className="px-4 pt-4 pb-8">
        ...
        <div className="flex items-center justify-between mb-3">
          <h2 className="font-display font-bold text-[21px] text-gray-900">Your Items</h2>
          <AddItemBtn storeId={store.id} onAdded={reload} toast={toast}/>
        </div>

        <div className="bg-white border border-gray-100 rounded-3xl overflow-hidden shadow-sm">
          {items.length === 0 && (
            <div className="p-6 text-center text-[17px] text-gray-400">
        ...
        <div className="mt-4 p-4 bg-white border border-gray-100 rounded-2xl shadow-sm">
          <h3 className="font-bold text-[17px] text-gray-900 mb-1">💡 How it works</h3>
        ...
        <div className="mt-3 p-4 bg-emerald-50 border border-emerald-100 rounded-2xl">
          <p className="text-[17px] text-emerald-700">
    ...
    <div className={`flex items-center gap-3 px-4 py-3 ${!isLast ? 'border-b border-gray-100' : ''}`}>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <span className="text-[17px] font-semibold text-gray-900 truncate">{item.name}</span>
          {!item.available && (
            <span className="text-[17px] font-bold px-1.5 py-0.5 rounded-full bg-red-50 text-red-500 flex-shrink-0">Hidden</span>
          )}
        </div>
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center" onClick={() => setOpen(false)}>
      <div className="w-full max-w-md bg-white rounded-t-3xl p-5" onClick={e => e.stopPropagation()}>
        ...
        <h3 className="font-display font-bold text-[21px] text-gray-900 mb-4">Add New Item</h3>
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-2">
            <span className="text-[17px] font-bold text-gray-500">£</span>
            <input type="number" value={price} onChange={e => setPrice(e.target.value)}
              className="flex-1 bg-gray-50 border border-gray-200 rounded-2xl px-4 py-3 text-[17px] outline-none focus:border-[#0f7a4b]"
